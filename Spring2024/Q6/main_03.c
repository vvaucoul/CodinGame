...
long long calculateOptimizedDistance(int *coords, int count) {
    qsort(coords, count, sizeof(int), compare);
    long long distance = 0;
    long long sum = 0;
    for (int i = 0; i < count; i++) {
        distance = (distance + (long long)i * coords[i] - sum) % MOD;
        sum = (sum + coords[i]) % MOD;
    }
    return distance;
}
...
int getCableLength(int n_rows, int n_cols, stringArray image) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        totalLength = (totalLength + calculateOptimizedDistance(xCoords, typeCounts[i])) % MOD;
        totalLength = (totalLength + calculateOptimizedDistance(yCoords, typeCounts[i])) % MOD;
        free(xCoords);
        free(yCoords);
    }
