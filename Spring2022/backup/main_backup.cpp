...
public:
    ...
    void moveToEntity(Entity *entity)
    {
        ...
        cout << "MOVE " << entity->get_location().x << " " << entity->get_location().y << " [" << _id << "] "
             << "-> [" << entity->get_id() << "]" << endl;
    }
    ...
    void cast_spell_control(Entity *entity, int vx, int vy)
    {
        if (_did_action == true)
            return;
        cout << "SPELL "
             << "CONTROL " << entity->get_id() << " " << vx << " " << vy << " [" << _id << "] "
             << "Wololoo" << endl;
        _action = E_HA_CAST_SPELL;
        _destination = {-1, -1};
    }
    ...
    vector<Entity *> sortEntities_by_distance_with_location(vector<Entity *> entities, Point location, float distance_max, bool inf_to_dist)
    {
        vector<Entity *> sortedEntities;
        for (Entity *entity : entities)
        {
    ...
    void setDidAction(bool did_action)
    {
        _did_action = did_action;
    }
    ...
    Point getRandomIALocation(int game_turn, Point base_location)
    {
        switch (_id)
        {
        case 0:
    ...
    pair<enum e_best_action, int> weightGetBestDefenseAction(vector<Entity *> entities, vector<Hero *> ennemy_heroes, Point base_location, size_t mana_ennemy)
    {
        ...
        int nb_ennemy_in_base = sortEntities_by_distance_with_location(entities, base_location, BASE_VIEW_DISTANCE, true).size();
...
Point Hero::getPointSeparateToHeroes(Game *game, float dist_max)
{
    ...
    {
        found = true;
        new_point = getRandomIALocation(game->getNbTurn(), game->getBaseLocation());
        for (Hero *hero : heroes)
        {
